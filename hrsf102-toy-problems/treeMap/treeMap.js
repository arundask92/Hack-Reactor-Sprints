...
Tree.prototype.map = function(callback) {
  ...
  var mapTree = function(oldTree, newTree, callback) {
    if (!!oldTree.children) {
      for (var i = 0; i < oldTree.children.length; i++) {
        var childTree = oldTree.children[i];
        var newValue = callback(childTree.value);
        var newBranch = newTree.addChild(newValue);
        if (!!childTree.children) {
          mapTree(childTree, newBranch, callback);
        }
      }
    }
  };
  var oldTree = this;
  mapTree(oldTree, newTree, callback);
  return newTree;
};
